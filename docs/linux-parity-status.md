...
- **Wave A（本轮，先行）**：上表 10 条崩溃链/毁灭性一行缺陷全部修复 + 可抓回归的测试
- **Wave B**：native ABI 扩展（显示器枚举/尺寸约束/DragMove/图标/状态查询/事件补发）+
  Wayland HiDPI/光标 + managed 接线（StartupLocation/Min-Max/DragMove/Topmost/SystemParameters 真数据）
- **Wave C**：媒体 GStreamer 运行时化 + 15s 阻塞 + SoundPlayerAction；通知 action 泵；
  portal Settings 深色模式；AT-SPI Value/EditableText；打印提交；剪贴板图像/HTML
- **Wave D**：Linux 示例应用；测试扩容（含 xvfb 真窗口 e2e）；CI 编排（孤儿脚本接入、
  Vulkan smoke、musl managed 测试、NuGet 消费验证）；README/docs 补齐
- **Wave E（后续跟进）**：xdg_popup、触摸输入、XShm/增量 present、LCD 亚像素、彩色 emoji、
  VAAPI 零拷贝、托盘 StatusNotifierItem、xdg-decoration、交叉编译 preset
