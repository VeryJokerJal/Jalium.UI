...
public sealed class NavigationService
{
    ...
    public bool Navigate(Uri source, object? extraData)
        => Navigate(source, extraData, sandboxExternalContent: false);

    /// <summary>
    /// Navigates to a URI with extra data and an external-content sandbox preference.
    /// </summary>
    /// <remarks>
    /// Sandboxing is consumed by platform navigation hosts. The managed journal keeps
    /// the value at this boundary so callers can use the WPF-compatible overload.
    /// </remarks>
    public bool Navigate(Uri source, object? extraData, bool sandboxExternalContent)
    {
        ArgumentNullException.ThrowIfNull(source);
        _ = sandboxExternalContent;
        ...
        out NavigatingCancelEventArgs args)
    {
        ...
        if (_content is Page currentPage)
        {
            currentPage.OnNavigatingFrom(args);
        }
    ...
    private void ApplyContent(Uri? source, object content, object? extraData, NavigationMode mode)
    {
        ...
        {
            oldPage.OnNavigatedFrom(new NavigationEventArgs(
                source,
                previous,
                extraData,
                webResponse: null,
