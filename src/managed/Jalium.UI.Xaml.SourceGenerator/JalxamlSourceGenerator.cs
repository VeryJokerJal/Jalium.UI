...
public sealed class JalxamlSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(combined, (spc, source) =>
        {
            ...
            GenerateAssemblyAotMetadata(spc, compilation, assemblyName);
        });
    }

    /// <summary>
    /// Emits one assembly-wide NativeAOT catalog. The runtime consumes this
    /// catalog instead of enumerating <see cref="System.Reflection.Assembly"/>
    /// metadata, and the generated registration contract keeps public binding
    /// members available to the reflection fallback used by string paths.
    /// </summary>
    private static void GenerateAssemblyAotMetadata(
        SourceProductionContext context,
        Compilation compilation,
        string assemblyName)
    {
        var registryType = compilation.GetTypeByMetadataName("Jalium.UI.AotTypeRegistry");
        if (registryType == null ||
            SymbolEqualityComparer.Default.Equals(
                registryType.ContainingAssembly,
                compilation.Assembly))
        {
            // The registry is unavailable, or this is the framework assembly
            // that defines it. Framework implementation types already carry
            // their own AOT annotations and must not be rooted wholesale.
            return;
        }

        var typeNames = EnumerateNamedTypes(compilation.Assembly.GlobalNamespace)
            .Select(GetAotRegistrableTypeName)
        ...
        var generatedTypeName =
            $"__JaliumGeneratedAotMetadata_{SanitizeIdentifier(assemblyName)}";
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
        sb.AppendLine("#pragma warning disable CS0612, CS0618, CS0619");
        sb.AppendLine();
        sb.AppendLine("namespace Jalium.UI.Generated;");
        sb.AppendLine();
        sb.AppendLine($"internal static class {generatedTypeName}");
        sb.AppendLine("{");
        ...
        {
            sb.AppendLine(
                $"        global::Jalium.UI.AotTypeRegistry.Register(typeof({typeName}));");
        }
        ...
        context.AddSource(
            "__JaliumGeneratedAotMetadata.g.cs",
        ...
        INamespaceSymbol namespaceSymbol)
    {
        ...
        {
            foreach (var candidate in EnumerateNamedTypes(type))
            {
        ...
        {
            foreach (var candidate in EnumerateNamedTypes(childNamespace))
            {
                yield return candidate;
            }
        }
        ...
        INamedTypeSymbol type)
    {
        ...
        {
            foreach (var candidate in EnumerateNamedTypes(nestedType))
            {
                yield return candidate;
            }
        }
    ...
    private static string? GetAotRegistrableTypeName(INamedTypeSymbol type)
    {
        ...
        for (var current = type; current != null; current = current.ContainingType)
        {
            ...
            if (!SymbolEqualityComparer.Default.Equals(current, type) &&
                current.Arity != 0)
            {
                // A nested type inside a generic containing type cannot be
                // expressed as one unbound typeof(...) operand without also
                // constructing the containing type.
                return null;
            }
        }

        var emittedType = type.Arity == 0
            ? type
            : type.ConstructUnboundGenericType();
        return emittedType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
    }
        ...
        bool deferFrameworkThemeStyles)
    {
        ...
        // For prebuilt-dictionary builders we cannot call XamlTypeRegistry.RegisterType(...)
    ...
    private string? GenerateCode(SourceProductionContext context, JalxamlParseResult result, string assemblyName, XmlnsTypeResolver xmlnsResolver, SymbolTypeHelper symbols, string sourceContent, string filePath, string? projectDir, string rootNs, string? jalxamlSourceRelativePath)
    {
        ...
        // De-duplicate by full qualified name so we never emit the same RegisterType call twice
        ...
        foreach (var element in result.ReferencedElements)
        {
            ...
            sb.AppendLine($"        global::Jalium.UI.Markup.XamlTypeRegistry.RegisterType(typeof({resolvedFullName}));");
        }
        ...
        foreach (var dataTypeFullName in dataTypeFullNames)
        {
            ...
            sb.AppendLine($"        global::Jalium.UI.Markup.XamlTypeRegistry.RegisterType(typeof({dataTypeFullName}));");
        }
        ...
        // been emitted in the ModuleInitializer above (RegisterType(typeof(T)) + DynamicDependency
...
public sealed class JalxamlParseResult
{
    ...
    /// <summary>
    /// Every runtime type referenced by the document: root/descendant element types and
    /// attached-property owner types, excluding XAML property-elements like
    /// <c>Grid.RowDefinitions</c>. Captured for AOT pinning so the generator can emit
    /// <c>RegisterType(typeof(T))</c> calls that preserve constructors and public attached
    /// property Set/Get methods. De-duplicated by (elementName, namespaceUri).
    /// </summary>
