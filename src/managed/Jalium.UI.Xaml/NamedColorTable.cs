...
/// <summary>
/// Shared lookup table covering every named color defined on <see cref="Colors"/> (the full
/// standard X11/WPF set). Used as a fallback by <see cref="BrushConverter"/> and
/// <see cref="ColorConverter"/> once their hand-tuned common-color fast path and the # hex
/// path have both missed, so a bare name such as "Crimson" or "ForestGreen" resolves
/// instead of throwing a <see cref="FormatException"/>.
///
/// Each entry references a <see cref="Colors"/> property directly. That is an ordinary
/// compile-time member access (never reflection), so the table is trim / NativeAOT safe and
/// its values are guaranteed identical to <see cref="Colors"/>. The entry list is generated
/// from Colors.cs - if a color is added or removed there, regenerate this table to match.
/// </summary>
internal static class NamedColorTable
{
    ...
    /// <summary>
    /// Looks up a standard named color by name (case-insensitive). Returns
    /// <see langword="true"/> with the matching color when <paramref name="name"/> is one
    /// defined on <see cref="Colors"/>; otherwise <see langword="false"/>.
    /// </summary>
    public static bool TryGet(string name, out Color color) => _colors.TryGetValue(name, out color);
}
