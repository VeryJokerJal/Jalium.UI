# Compiles blur_quad.frag.hlsl to SPIR-V (DXC) and SURGICALLY replaces
# ONLY the kBlurQuadFragmentShaderSpv array inside
# ../include/vulkan_embedded_shaders.h (splice pattern of gen_backdrop_quad_spv.ps1;
# the vertex shader is untouched here, so its array is left byte-identical).
#
# blur_quad.frag uses register(t0)/register(s1) with NO shift flags — same
# convention as backdrop_quad.frag / bitmap_quad.frag — so the plain
# -spirv -T ps_6_0 -E main -O3 flags are correct (register numbers map directly
# to descriptor binding indices; the blur pipeline binds frameDescriptorSet:
...
$fs = Compile-Spv (Join-Path $ShaderSrcDir 'blur_quad.frag.hlsl') 'ps_6_0'
...
$text = Splice-Array $text 'kBlurQuadFragmentShaderSpv' $fsBody
...
Write-Host ("Spliced kBlurQuad frag SPIR-V into {0}  (frag={1}B)" -f $HeaderPath, $fs.Length)
