# Compiles the Vello composite VS/FS (vello_composite.{vert,frag}.hlsl) to SPIR-V
# and embeds them into ../include/vulkan_vello_composite_shaders.h. The fragment
# shader uses explicit [[vk::binding]] so no register-class shift is required.

param(
    [string]$ShaderSrcDir = $PSScriptRoot,
    [string]$HeaderPath   = (Join-Path $PSScriptRoot '..\include\vulkan_vello_composite_shaders.h')
)
...
$vs = Compile-Spv (Join-Path $ShaderSrcDir 'vello_composite.vert.hlsl') 'vs_6_0'
$fs = Compile-Spv (Join-Path $ShaderSrcDir 'vello_composite.frag.hlsl') 'ps_6_0'

$sb = [System.Text.StringBuilder]::new()
[void]$sb.AppendLine('#pragma once')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('// Auto-generated by shaders/gen_vello_composite_spv.ps1 - do not edit manually.')
[void]$sb.AppendLine('// Vello output-image composite VS/FS (fullscreen triangle + premultiplied sample).')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('#include <cstddef>')
[void]$sb.AppendLine('#include <cstdint>')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('namespace jalium {')
Emit-Array $sb 'kVelloCompositeVertSpv' $vs
Emit-Array $sb 'kVelloCompositeFragSpv' $fs
[void]$sb.AppendLine('')
[void]$sb.AppendLine('} // namespace jalium')
