...
public sealed class ComponentModelNamespaceParityTests
{
    [Fact]
    ...
    public void CanonicalGroupingOwnsSortAndCustomComparerSemantics()
    {
        ...
        group.SortDescriptionsInGrouping = true;
        ...
        Assert.Contains(nameof(CanonicalGroupDescription.SortDescriptionsInGrouping), changed);
    ...
    [Fact]
    public void RemovedLegacyComponentModelAliasesDoNotRemainInTheManagedAssembly()
    {
        var assembly = typeof(CollectionView).Assembly;
        string[] legacyTypes =
        [
            ...
            "Jalium.UI.ICollectionViewLiveShaping",
            "Jalium.UI.IEditableCollectionViewAddNewItem",
        ];

        foreach (string legacyType in legacyTypes)
        {
            Assert.Null(assembly.GetType(legacyType));
        }
    }

    [Fact]
    public void JaliumGenericGroupComparerBridgeUsesTheCanonicalGroupDescription()
    {
        var group = new CanonicalProbeGroup();
        var comparer = Comparer<object>.Create((left, right) =>
            string.CompareOrdinal(left?.ToString(), right?.ToString()));

        group.SetJaliumCustomSort(comparer);

        Assert.Same(comparer, group.GetJaliumCustomSort());
    }
    ...
    public void CanonicalWeakEventManagersFilterAndDetachHandlers()
    {
        ...
        CanonicalPropertyChangedEventManager.AddListener(propertySource, listener);
        propertySource.Raise(nameof(PropertySource.Other));
        CanonicalPropertyChangedEventManager.RemoveListener(propertySource, listener);
    ...
    public void CanonicalDependencyDescriptorAndDesignerPropertyAreFunctional()
    {
        ...
        var category = Assert.Single(
            typeof(CanonicalDesignerProperties)
                .GetField(nameof(CanonicalDesignerProperties.IsInDesignModeProperty))!
                .GetCustomAttributes(typeof(DevToolsPropertyCategoryAttribute), inherit: false)
                .Cast<DevToolsPropertyCategoryAttribute>());
        Assert.Equal(DevToolsPropertyCategory.State, category.Category);
    }
