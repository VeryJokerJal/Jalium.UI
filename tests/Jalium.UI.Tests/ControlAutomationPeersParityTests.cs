...
public sealed class ControlAutomationPeersParityTests
{
    [Fact]
    ...
    public void ComboBoxPeer_PreservesSelectionExpandAndEditableValuePatterns()
    {
        ...
        Assert.Same(peer, peer.GetPattern(PatternInterface.Selection));
        var value = Assert.IsAssignableFrom<Raw.IValueProvider>(peer.GetPattern(PatternInterface.Value));
    ...
    public void RangeAndPasswordPeers_ExposeWpfSemantics()
    {
        ...
        Assert.Equal(4, Assert.IsAssignableFrom<Raw.IRangeValueProvider>(scrollPeer.GetPattern(PatternInterface.RangeValue)).Value);
