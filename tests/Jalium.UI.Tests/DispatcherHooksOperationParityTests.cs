...
public sealed class DispatcherHooksOperationParityTests
{
    [Fact]
    public void HooksExposeTypedOperationLifecycleAndInactiveNotifications()
    {
        ...
        DispatcherOperation? trackedOperation = null;

        DispatcherHookEventHandler posted = (sender, args) =>
        {
            // BeginInvoke raises OperationPosted before it returns the operation.
            // Capture that first operation and ignore unrelated render work that
            // another window may enqueue while this dispatcher drains its queue.
            trackedOperation ??= args.Operation;
            if (!ReferenceEquals(args.Operation, trackedOperation)) return;
            order.Add("posted");
        ...
        try
        {
            ...
            Assert.Same(operation, trackedOperation);
