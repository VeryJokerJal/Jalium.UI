...
public sealed class LayeredNamespaceFacetParityTests
{
    [Fact]
    ...
    public void DataGrid_OverridesSharedSelectionHook()
    {
        ...
        Assert.Equal(typeof(MultiSelector), typeof(DataGrid).BaseType);
        Assert.Equal(typeof(Selector), method.GetBaseDefinition().DeclaringType);
    ...
    [Fact]
    public void FlowDocument_ExposesCanonicalPaginatorAndViewerUsesCanonicalDocumentsContract()
    {
        Type type = typeof(FlowDocument);
        Assert.Contains(typeof(Jalium.UI.Documents.IDocumentPaginatorSource), type.GetInterfaces());
        Assert.Equal(
            typeof(Jalium.UI.Documents.IDocumentPaginatorSource),
            typeof(DocumentViewerBase).GetProperty(nameof(DocumentViewerBase.Document))!.PropertyType);

        var document = new FlowDocument();
        var viewer = new FlowDocumentPageViewer { Document = document };
        Assert.Same(document, ((DocumentViewerBase)viewer).Document);
        Assert.IsAssignableFrom<Jalium.UI.Documents.DocumentPaginator>(
            ((DocumentViewerBase)viewer).Document!.DocumentPaginator);
    }

    [Fact]
    public void LegacyDocumentPaginationCopies_AreNotPubliclyExported()
    {
        Type[] exportedTypes = typeof(DocumentPage).Assembly.GetExportedTypes();
        ...
        foreach (string legacyTypeName in legacyTypeNames)
        {
            Assert.DoesNotContain(exportedTypes, type => type.FullName == legacyTypeName);
        }

        Assert.Contains(typeof(DocumentPage), exportedTypes);
        Assert.Contains(typeof(DocumentPaginator), exportedTypes);
        Assert.Contains(typeof(IDocumentPaginatorSource), exportedTypes);
    }
