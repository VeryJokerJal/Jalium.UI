...
public sealed class MediaElementRemainingParityTests
{
    [Fact]
    ...
    [Fact]
    public void AudioControlsPropagateToTheRealAudioPipeline()
    {
        using var element = new MediaElement
        {
            Balance = -0.4,
        ...
        var field = typeof(MediaElement).GetField(
            "_audioManager",
            BindingFlags.Instance | BindingFlags.NonPublic);
        var player = Assert.IsType<AudioPlayer>(field!.GetValue(element));

        Assert.Equal(-0.4, player.Balance, 6);
        Assert.Equal(1.75, player.SpeedRatio, 6);
        Assert.Equal(0.6, player.Volume, 6);
