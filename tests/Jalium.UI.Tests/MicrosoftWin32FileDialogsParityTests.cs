...
public class MicrosoftWin32FileDialogsParityTests
{
    [Fact]
    public void CanonicalTypes_HaveWpfHierarchyAndSurface()
    {
        ...
        MethodInfo ownerOverload = typeof(CanonicalCommonDialog).GetMethod(
            ...
            [typeof(Jalium.UI.Window)])!;
        Assert.Equal(typeof(bool?), ownerOverload.ReturnType);

        MethodInfo nativeOwnerOverload = typeof(CanonicalFileDialog).GetMethod(
            nameof(CanonicalFileDialog.ShowDialog),
            [typeof(IntPtr)])!;
        Assert.Equal(typeof(bool?), nativeOwnerOverload.ReturnType);
        Assert.Equal(typeof(bool), typeof(CanonicalFileDialog)
            .GetProperty(nameof(CanonicalFileDialog.IsPortalAvailable))!.PropertyType);
        Assert.Equal(typeof(TimeSpan), typeof(CanonicalFileDialog)
            .GetProperty(nameof(CanonicalFileDialog.PortalTimeout))!.PropertyType);
        Assert.Equal(typeof(CancellationToken), typeof(CanonicalFileDialog)
            .GetProperty(nameof(CanonicalFileDialog.CancellationToken))!.PropertyType);

        Assembly assembly = typeof(CanonicalFileDialog).Assembly;
        Assert.False(assembly.GetType("Jalium.UI.Controls.FileDialog", throwOnError: true)!.IsPublic);
        Assert.False(assembly.GetType("Jalium.UI.Controls.OpenFileDialog", throwOnError: true)!.IsPublic);
        Assert.False(assembly.GetType("Jalium.UI.Controls.SaveFileDialog", throwOnError: true)!.IsPublic);
        Assert.False(assembly.GetType("Jalium.UI.Controls.FileDialogCustomPlace", throwOnError: true)!.IsPublic);
    }
    ...
    public void FileDialogs_DefaultAndResetStateMatchesWpf()
    {
        ...
        Assert.Equal(TimeSpan.FromMinutes(2), open.PortalTimeout);
        Assert.Equal(default(CancellationToken), open.CancellationToken);
        ...
        open.PortalTimeout = TimeSpan.FromSeconds(3);
        open.CancellationToken = new CancellationToken(canceled: true);
        open.Reset();
