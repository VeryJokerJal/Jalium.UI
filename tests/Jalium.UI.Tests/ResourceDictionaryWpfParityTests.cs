...
public sealed class ResourceDictionaryWpfParityTests
{
    [Fact]
    public void PublicSurfaceExposesTierOneResourceDictionaryApis()
    {
        ...
        Assert.Contains(typeof(INameScope), dictionaryType.GetInterfaces());
        Assert.Equal(
    ...
    public void NameScopeRegistersFindsAndUnregistersObjects()
    {
        ...
        Assert.Same(value, ((INameScope)dictionary).FindName("resourcePart"));
    ...
    public void CopyAndEnumerationPathsRunOnGettingValue()
    {
        ...
        var pairs = new KeyValuePair<object, object?>[1];
        dictionary.CopyTo(pairs, 0);
        Assert.Equal("raw:2", pairs[0].Value);

        using IEnumerator<KeyValuePair<object, object?>> genericEnumerator = dictionary.GetEnumerator();
        Assert.True(genericEnumerator.MoveNext());
        Assert.Equal("raw:3", genericEnumerator.Current.Value);

        IEnumerator<object?> valuesEnumerator = dictionary.Values.GetEnumerator();
        Assert.True(valuesEnumerator.MoveNext());
        Assert.Equal("raw:4", valuesEnumerator.Current);
        ...
        Assert.Equal("raw:5", dictionaryEnumerator.Value);

        Assert.Equal(5, dictionary.GettingValueCallCount);
    }
