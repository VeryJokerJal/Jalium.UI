...
public sealed class SizeParityTests
{
    [Fact]
    ...
    public void LayoutZeroAndDocumentMissingSentinels_RemainDistinct()
    {
        ...
        var pageVisual = new Border();
        var mutableDocumentPage = new Jalium.UI.Documents.DocumentPage
        {
            Size = Size.Empty,
            Visual = pageVisual,
        };
        Assert.True(mutableDocumentPage.Size.IsEmpty);
        Assert.Same(pageVisual, mutableDocumentPage.Visual);
        Assert.True(Jalium.UI.Documents.DocumentPage.Missing.Size.IsEmpty);
    ...
    private sealed class EmptyPagePaginator : Jalium.UI.Documents.DocumentPaginator,
        Jalium.UI.Documents.IDocumentPaginatorSource
    {
        public override int PageCount => 1;
        public override bool IsPageCountValid => true;
        public override Size PageSize { get; set; } = new(800, 600);
        public override Jalium.UI.Documents.IDocumentPaginatorSource Source => this;

        Jalium.UI.Documents.DocumentPaginator Jalium.UI.Documents.IDocumentPaginatorSource.DocumentPaginator => this;

        public override Jalium.UI.Documents.DocumentPage GetPage(int pageNumber) =>
            new() { Size = Size.Empty };
    }
}
