...
public sealed class SystemFontsParityTests
{
    ...
    public void PublicSurface_HasAllWpfFontValuesAndResourceKeys()
    {
        ...
        Assert.Null(typeof(SystemFonts).Assembly.GetType("Jalium.UI.Data.SystemFonts", throwOnError: false));
    ...
    public void Values_AreUsableAndReferenceValuesRemainStable()
    {
        foreach (var kind in Names(FontKinds))
        {
            ...
            Assert.True(decorations.IsFrozen);
            Assert.All(
                decorations,
    ...
    [Fact]
    public async Task SharedTextDecorations_AreFreeThreaded()
    {
        TextDecorationCollection[] expected =
        [
            SystemFonts.IconFontTextDecorations,
            SystemFonts.CaptionFontTextDecorations,
            SystemFonts.SmallCaptionFontTextDecorations,
            SystemFonts.MenuFontTextDecorations,
            SystemFonts.StatusFontTextDecorations,
            SystemFonts.MessageFontTextDecorations,
        ];

        int[] counts = await Task.Run(() =>
            expected.Select(static decorations => decorations.Count).ToArray());

        Assert.Equal(expected.Length, counts.Length);
        for (var index = 0; index < expected.Length; index++)
        {
            Assert.True(expected[index].IsFrozen);
            Assert.Equal(expected[index].Count, counts[index]);
        }
    }
